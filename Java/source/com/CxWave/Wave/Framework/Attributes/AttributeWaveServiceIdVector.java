...
public class AttributeWaveServiceIdVector extends Attribute
{
    private static final Vector<WaveServiceId> s_temp = null;
    ...
    public Set<String> getSupportedDataTypes ()
    {
        ...
        {
            WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_FATAL, "AttributeWaveServiceIdVector.getSupportedDataTypes : Could not return supported data type.  Exception : " + e.toString ());
    ...
    public void loadValueFromPlainString (final Vector<WaveServiceId> value, final SerializableObject serializableObject)
    {
        ...
        try
        {
            ...
            if (null == object)
            {
                ...
                {
                    WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_FATAL, "AttributeWaveServiceIdVector.loadValueFromPlainString : Could not find constructor with 0 arguments for Field Type : %s", fieldType.getName ());
                    WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_FATAL, "AttributeWaveServiceIdVector.loadValueFromPlainString :     Please declare a constructor with no arguments and retry.");
                ...
                else
                {
                    ...
                    {
                        WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_FATAL, "AttributeWaveServiceIdVector.loadValueFromPlainString:  Instantiating fieldType :%s failed.  Status : %s", fieldType.getName (), e.toString ());
                        WaveAssertUtils.waveAssert ();
                    }
                }
            }
        ...
        {
            WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_INFO, "AttributeWaveServiceIdVector.loadValueFromPlainString : Attribute loading failed for Field : %s, Class : %s, value : %s, Status : %s", m_reflectionAttribute.getAttributeName (), (serializableObject.getClass ()).getName (), value, e.toString ());
            WaveAssertUtils.waveAssert ();
        }
    }
    ...
    public void loadValueFromWaveConfigurationFile (final WaveConfigurationFile waveConfigurationFile, final SerializableObject serializableObject, final String xmlWaveXPathPrefix)
    {
        ...
        {
            WaveTraceUtils.tracePrintf (TraceLevel.TRACE_LEVEL_INFO, "AttributeWaveServiceIdVector.loadValueFromWaveConfigurationFile : Field : %s, Class : %s, xmlWaveXPathValue : %s", m_reflectionAttribute.getAttributeName (), serializableObject.getClass ().getName (), xmlWaveXPathValue);
            ...
            {
                final Vector<WaveServiceId> value = new Vector<WaveServiceId> ();

                for (final String configurationValue : configurationValues)
                {
                    if (WaveStringUtils.isNotBlank (configurationValue))
                    {
                        value.add (new WaveServiceId (new UI32 (configurationValue)));
                    }
                }
    ...
    public void toWaveString (final SerializableObject thisSerializableObject, final StringBuffer value)
    {
        ...
        final Vector<WaveServiceId> data = (Vector<WaveServiceId>) object;

        WaveAssertUtils.waveAssert (null != data);

        for (final WaveServiceId waveServiceId : data)
        {
            value.append ("#");
            value.append (waveServiceId.toWaveString ());
        }
    }
    ...
    public void fromWaveString (final SerializableObject thisSerializableObject, final String value)
    {
        ...
        if (null == object)
        {
            ...
            {
                object = new Vector<WaveServiceId> ();
            }
            else
            {
                return;
        ...
        while (true)
        {
            ...
            final WaveServiceId waveServiceId = new WaveServiceId (new UI32 (0));
            waveServiceId.fromWaveString (valueString);

            data.add (waveServiceId);
        }
    }
}

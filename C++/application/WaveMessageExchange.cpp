...
extern "C" int waveMessageBrokerMain (int argc, char *argv[]);
...
int main (int argc, char *argv[])
{
    ...
    int   i;
    int   numberOfCommandLineArguments    = 33;
    ...
    strcpy (pCommandLineArguments[31], "19110");
    ...
    for (i = 1; i < argc; i += 2)
    {
        ...
        else if (0 == strcasecmp ("-messagebrokerport",  argv[i]))
        {
            strcpy (pCommandLineArguments[31], argv[i + 1]);
        }
        else if (0 == strcasecmp ("-messagebrokerclient",  argv[i]))
        {
            strcpy (pCommandLineArguments[32], argv[i + 1]);
        }
        else
        {
            cerr << endl;
    ...
    return (waveMessageBrokerMain (numberOfCommandLineArguments, pCommandLineArguments));
}

...
ResourceId FrameworkToolKit::analyzeJson (UI32 argc, vector<string> argv)
{
    ...
    if (argc >= 2)
    {
        ...
        JsonValue *pJsonValue = JsonFactory::createJsonValueFromString (jsonString);

        if (NULL != pJsonValue)
        {
            bool successFullyLoaded = pJsonValue->getSuccessFullyLoaded ();

            tracePrintf (TRACE_LEVEL_INFO, true, false, "Successfully Loaded : %s", successFullyLoaded ? "True" : "False");

            string jsonOutput = pJsonValue->toString ();

            trace (TRACE_LEVEL_INFO, jsonOutput);
...
ResourceId FrameworkToolKit::debugTokenizeConsideringStringQuotes (UI32 argc, vector<string> argv)
{
    if (argc >= 2)
    {
        UI32 i = 0;
        ...
        for (i = 1; i < argc; i++)
        {
            ...
            StringUtils::tokenizeConsideringStringQuotes (input, tokens, ':');
